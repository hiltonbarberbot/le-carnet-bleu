...
export type StorylineAuthoringAttempt =
  | { status: 'drafted'; definition: ReturnType<typeof createStorylineDefinition> }
...
export async function authorStorylineAttempt(
  ...
  try {
    const draft = output && typeof output === 'object' && !Array.isArray(output)
      ? { ...output, setting }
      : output
    return { status: 'drafted', definition: createStorylineDefinition(draft as StorylineDefinitionInput) }
  } catch (error) {
    return {
      status: 'rejected',
