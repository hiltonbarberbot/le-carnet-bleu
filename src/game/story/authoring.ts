...
9. Keep physical opening instructions no-contact, reversible, host-cued, and achievable by a named host proxy.
...
11. Do not invent a second player task model for AI roles; their play is governed by the same objectives, relationships, and secrets.
12. Create exactly two clue decks tied to verified setting resource IDs and exactly five purchasable clues total. Each clue names the solution-step IDs it supports, but every solution step must retain two independently sourced non-purchasable evidence routes.
...
14. Keep five to eight entries in one ordered openingSteps checklist. Do not add phases or a dependency graph; array order is play order. It is a cold open, not a guided first half of the game.
15. Return a schema-v5 StorylineDefinitionInput containing id, title, setting, story, clueDecks, acts, and setupRequirements. Preserve every stable setting resource ID and pass the result through createStorylineDefinition before constructing a runtime.`
}
