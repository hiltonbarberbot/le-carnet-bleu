...
describe('truthful game lifecycle', () => {
  ...
  it('runs a complete gated playthrough only after confirmed delivery', () => {
    ...
    expect(active.playPhase).toBe('schemes')
    expect(() => advanceAct(definition, active)).toThrow(/Arrival and first schemes is missing/)
    for (const beat of story.runPlan.filter(beat => beat.phase === 'schemes' && beat.essential)) active = confirmRunBeat(definition, active, beat.id)
    active = advanceAct(definition, active)
    expect(() => advanceAct(definition, active)).toThrow(/public reckoning is missing/i)
    for (const beat of story.runPlan.filter(beat => beat.phase === 'reckoning' && beat.essential)) active = confirmRunBeat(definition, active, beat.id)
    active = advanceAct(definition, active)
