...
const story = generateGame('ui')
const noAi = { aiControllers: false }

function enrolling() {
  let state = createGame(story, new Date('2026-08-18T10:00:00Z'), 'ui-game')
...
describe('God mode lifecycle projection', () => {
  it('renders first load as idle with create but no reset action', () => {
    const html = render(createIdleState(story))
    expect(html).toContain('IDLE · NO GAME')
  ...
  it('renders partial enrolment as blocked', () => {
    const html = render(createGame(story, new Date('2026-08-18T10:00:00Z'), 'partial'))
  ...
  it('renders prepared-but-unsent and failed delivery distinctly', () => {
    let prepared = prepareGame(story, enrolling(), noAi)
  ...
  it('renders active play and reset-to-idle as separate states', () => {
    const active = startGame(story, deliverAll(prepareGame(story, enrolling(), noAi)))
    expect(getHostScreen(active)).toBe('active:dinner')
    expect(render(active)).toContain('Run the causal chain')
    const idle = resetGame(story, active, true)
    expect(getHostScreen(idle)).toBe('idle')
    expect(render(idle)).toContain('IDLE · NO GAME')
  })
  ...
  it('exposes Gateway performances for an active AI seat', () => {
    ...
    const active = startGame(story, deliverAll(prepareGame(story, withVacancy, { aiControllers: true })))
