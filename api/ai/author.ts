  ...
  for (let attempt = 0; attempt < 2; attempt += 1) {
    try {
      const result = await generateText({
        ...
        output: Output.json({ name: 'setting_specific_game_definition', description: 'A complete validated Le Carnet Bleu game definition.' }),
        maxOutputTokens: 12000,
        temperature: 0.7,
        providerOptions: { gateway: { tags: ['le-carnet-bleu', 'story-authoring'] } },
      })
